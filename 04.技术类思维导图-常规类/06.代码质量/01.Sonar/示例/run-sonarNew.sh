...
codesign=''; readParameter codesign 'sonar.objectivec.codesign'
#codesign="\'iPhone Distribution: 58 daojia Life Service Co.,Ltd. (SYNJQ6CWNM)\'"
profilename=''; readParameter profilename 'sonar.objectivec.profilename'
...
if [ "$vflag" = "on" ]; then
 	...
 	echo "Xcode codesign is: $codesign"
 	echo "Xcode profilename is: $profilename"
...
if [[ "$workspaceFile" != "" ]] ; then
	...
	# 16/12/6 修改: xctool不支持xcode8, 改用xcodebuildbuild
	xcodebuildCmdPrefix="xcodebuild -workspace $workspaceFile -scheme ${appScheme} CODE_SIGN_IDENTITY=${codesign} PROVISIONING_PROFILE=${profilename} ONLY_ACTIVE_ARCH=NO -configuration Release "
#else
	...
	#xcodebuildCmdPrefix="xcodebuild -project $projectFile -scheme ${appScheme} CODE_SIGN_IDENTITY=${codesign} PROVISIONING_PROFILE=${profilename} ONLY_ACTIVE_ARCH=NO -configuration Release"
fi	
...
#else
	#xcodebuild clean
	#xcodebuild -project "${projectFile}" -configuration Debug  build | tee xcodebuild.log | xcpretty -r json-compilation-database --output compile_commands.json
#fi
...
if [ "$oclint" = "on" ]; then
	...
	oclint-json-compilation-database -v -e Pods -e COMB/Tools -- -max-priority-1 $maxPriority -max-priority-2 $maxPriority -max-priority-3 $maxPriority -rc LONG_LINE=150 -report-type pmd -o oclint.xml
